...
export const todayISO = () => {
  const wib = new Date(startOfConversionDay() + WIB);
...
export function rangeBounds(range, from, to) {
  ...
  switch (range) {
    ...
    case "custom": {
      ...
      const fromDate = new Date(from + "T00:00:00Z").getTime() - WIB + DAY_START_MS;
      const toDate = new Date(to + "T00:00:00Z").getTime() - WIB + DAY_START_MS + 86400000 - 1;
